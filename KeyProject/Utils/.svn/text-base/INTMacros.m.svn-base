...
UIImage *navigationBarBackgroundImagePortrait;
UIImage *navigationBarBackgroundImageLandscape;
UIImage *toolBarBackgroundImage;
UIImage *tabBarSelectionIndicatorImage;

//For iPhone
UIImage *navigationBarBGImage_Portrait()
{
    if (!navigationBarBackgroundImagePortrait) {
        UIColor *color = HEX_COLOR_STRING(@"#0058ad");
        CGRect rect = CGRectMake(0, 0, 320, 44);
        UIGraphicsBeginImageContextWithOptions(rect.size, NO, 0);
        [color setFill];
        UIRectFill(rect);   // Fill it with your color
        navigationBarBackgroundImagePortrait = UIGraphicsGetImageFromCurrentImageContext();
        UIGraphicsEndImageContext();
    }
    return navigationBarBackgroundImagePortrait;
}

UIImage *navigationBarBGImage_Landscape()
{
    if (!navigationBarBackgroundImageLandscape) {
        UIColor *color = HEX_COLOR_STRING(@"#0058ad");
        CGRect rect = CGRectMake(0, 0, 568, 32);
        UIGraphicsBeginImageContextWithOptions(rect.size, NO, 0);
        [color setFill];
        UIRectFill(rect);   // Fill it with your color
        navigationBarBackgroundImageLandscape = UIGraphicsGetImageFromCurrentImageContext();
        UIGraphicsEndImageContext();
    }
    return navigationBarBackgroundImageLandscape;
}
...
UIImage *toolBarBGImage()
{
    if (!toolBarBackgroundImage) {
        ...
        [color setFill];
        UIRectFill(rect);   // Fill it with your color
        toolBarBackgroundImage = UIGraphicsGetImageFromCurrentImageContext();
        UIGraphicsEndImageContext();
    }
...
UIImage* IMAGE_FROM_COLOR_RECT(UIColor *color, CGRect rect)
{
    ...
    UIImage *image = nil;
    switch (height) {
        case 34://navigation bar landscape
            if (!navigationBarBackgroundImageLandscape) {
                UIGraphicsBeginImageContextWithOptions(rect.size, NO, 0);
                [color setFill];
                UIRectFill(rect);   // Fill it with your color
            ...
            image = navigationBarBackgroundImageLandscape;
            break;
            
        case 44://navigation bar portrait
            if (!navigationBarBackgroundImagePortrait) {
                UIGraphicsBeginImageContextWithOptions(rect.size, NO, 0);
                [color setFill];
                UIRectFill(rect);   // Fill it with your color
            ...
            image = navigationBarBackgroundImagePortrait;
            break;
        ...
        case 45://toolBar
            ...
            image = toolBarBackgroundImage;
            break;
            
        case 49://toolBar selection indicator
            if (!tabBarSelectionIndicatorImage) {
                UIGraphicsBeginImageContextWithOptions(rect.size, NO, 0);
                [color setFill];
                UIRectFill(rect);   // Fill it with your color
                tabBarSelectionIndicatorImage = UIGraphicsGetImageFromCurrentImageContext();
                UIGraphicsEndImageContext();
            }
            image = tabBarSelectionIndicatorImage;
            break;
...
/*
UIImage *buyColorImage;
UIImage *sellColorImage;

UIImage* BUY_IMAGE_COLOR()
{
    if (!buyColorImage) {
        buyColorImage = IMAGE_FROM_COLOR(kButtonBuyTextColor);
    }
    return buyColorImage;
}

UIImage* SELL_IMAGE_COLOR()
{
    if (!sellColorImage) {
        sellColorImage = IMAGE_FROM_COLOR(kButtonSellTextColor);
    }
    return sellColorImage;
}
*/
...
*/
NSInteger AVERAGE_TIME(int time1, int time2)
{
    int total = (time1 + time2) /2;
    NSMutableString *stringTime =[NSMutableString stringWithFormat:@"%d",total];
    if (stringTime.length == 5) {
        [stringTime insertString:@"0" atIndex:0];
    }
    int second = [[stringTime substringWithRange:NSMakeRange(4, 2)] integerValue];
    int minus = [[stringTime substringWithRange:NSMakeRange(2, 2)] integerValue];
    int hours = [[stringTime substringWithRange:NSMakeRange(0, 2)] integerValue];
    if (second >= 60) {
        second = second % 60;
        minus ++;
    }
    ...
    if (minus >= 10)
        minusString = [NSString stringWithFormat:@"%d",minus];
    else
        minusString = [NSString stringWithFormat:@"0%d",minus];
    ...
    NSString *str = [NSString stringWithFormat:@"%@%@%@",hoursString, minusString, secondString];
