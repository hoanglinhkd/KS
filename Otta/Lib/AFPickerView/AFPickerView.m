...
- (void)reloadData
{
    ...
    contentView.contentSize = CGSizeMake(contentView.frame.size.width, 39.0 * rowsCount + 2 * 39.0);
    [self tileViews];
}
...
- (BOOL)isDisplayingViewForIndex:(NSUInteger)index
{
    ...
    for (UIView *aView in visibleViews) 
	{
        int viewIndex = aView.frame.origin.y / 39.0 - 1;
        if (viewIndex == index) 
		{
            foundPage = YES;
            break;
...
- (void)configureView:(UIView *)view atIndex:(NSUInteger)index
{
    ...
    frame.origin.y = 39.0 * index + 39.0f;
    label.frame = frame;
}
